...
function printHeader(text = "") {
    ...
    const centered = txt.length >= width
        ? txt
        : " ".repeat(Math.floor((width - txt.length) / 2)) + txt + " ".repeat(Math.ceil((width - txt.length) / 2));
...
function main() {
    try {
        ...
        console.log(key + "\n");
        print_info(`${Colors.BLUE}Save the hash of the API Key to config.yaml file? ${Colors.ENDC}(${Colors.ITALIC}${Colors.BOLD}${Colors.GREEN}Y${Colors.ENDC}/${Colors.RED}n${Colors.ENDC})`);
        ...
        readline.question("", choice => {
            if (choice.trim() === "Y" || choice.trim() === "y" || choice.trim() === "") {
                try {
                    ...
                    print_error("Failed to save API Key to config.yaml:"); console.log(e);
                }
            } else {
                readline.close();
            }
